    ...
    for t in tqdm(range(0, num_epoch)):
        ...
        if t % 100 == 0:
            ...
            with torch.no_grad():
                ...
                axs[0].scatter(
                    dataset[:300, 0], dataset[:300, 1], color="blue", edgecolor="white"
                )
                axs[1].scatter(
                    out[:300, 0], out[:300, 1], color="red", edgecolor="white"
                )
                plt.savefig(
                    f"rl-toolkit/dm/trained_imgs/{env}-pos_norm_{args.lr}_{args.hidden_dim}-{args.depth}.png"
                )
                plt.close()
        if t % 20 == 0:
            ...
            plt.savefig(
                f"rl-toolkit/dm/trained_imgs/{env}_ddpm_norm_loss_{args.lr}_{args.hidden_dim}-{args.depth}.png"
            )
            plt.close()
        if t % 1000 == 0:
            torch.save(
                model.state_dict(),
                f"rl-toolkit/dm/trained_models/{env}_ddpm_norm_{args.lr}_{args.hidden_dim}-{args.depth}.pt",
            )
    torch.save(
        model.state_dict(),
        f"rl-toolkit/dm/trained_models/{env}_ddpm_norm_{args.lr}_{args.hidden_dim}-{args.depth}.pt",
