...
class MazeGenFarGoalEval3(mujoco_env.MujocoEnv, utils.EzPickle, offline_env.OfflineEnv):
    def __init__(
        self, maze_spec=GEN, reward_type="dense", reset_target=False, **kwargs
    ):
    ...
    def step(self, action):
        ...
        if self.reward_type == "sparse":
            reward = 1.0 if np.linalg.norm(ob[0:2] - self._target) <= 0.4 else 0.0
        elif self.reward_type == "dense":
            # reward = np.exp(-np.linalg.norm(ob[0:2] - self._target))
            reward = self.last_dis - (np.linalg.norm(ob[0:2] - self._target))
            self.last_dis = (np.linalg.norm(ob[0:2] - self._target))
        else:
            raise ValueError("Unknown reward type %s" % self.reward_type)
        # done = False

        # Add goal_achieved to info dict
        goal_achieved = True if np.linalg.norm(ob[0:2] - self._target) <= 0.4 else False
        x = ob[0]
        y = ob[1]
    ...
    def reset_model(self):
        ### NOTE ###
        # why noise_ratio is changed before reset?
    ...
    def is_reached(self, ob):
        ...
        if isinstance(ob, torch.Tensor):
            return 1.0 if np.linalg.norm(ob[0:2].cpu() - ob[-2:].cpu()) <= 0.4 else 0.0
        return 1.0 if np.linalg.norm(ob[0:2] - ob[-2:]) <= 0.1 else 0.0
