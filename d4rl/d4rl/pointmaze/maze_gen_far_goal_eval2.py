...
class MazeGenFarGoalEval2(mujoco_env.MujocoEnv, utils.EzPickle, offline_env.OfflineEnv):
    def __init__(
    ...
    def step(self, action):
        ...
        if self.reward_type == "sparse":
            reward = 1.0 if np.linalg.norm(ob[0:2] - self._target) <= 0.3 else 0.0
    ...
    def is_reached(self, ob):
        ...
        if isinstance(ob, torch.Tensor):
            return 1.0 if np.linalg.norm(ob[0:2].cpu() - ob[-2:].cpu()) <= 0.3 else 0.0
        return 1.0 if np.linalg.norm(ob[0:2] - ob[-2:]) <= 0.3 else 0.0
